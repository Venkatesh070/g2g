...
class PaymentStatusController extends GetxController {
  ...
  void onInit() {
    ...
    if (paymentType.value == "intent") {
      Future.delayed(Duration.zero, () async {
        try {
          launchUrl((Uri.parse(
              createIntentData.value.payment!.intentUrl.toString())));
        } catch (exception) {
  ...
  paymentStatusCheck(apiCheckTime) async {
    try {
      ...
      ApiResponseModel<CreateIntentModel> paymentStatusCheck =
          await DioClient.base(accessToken: accessToken)
              .funCheckIntentStatusApi(dataParams);

      if (paymentStatusCheck.success!) {
        if (paymentStatusCheck.data!.payment!.status == 'completed') {
          pDescription.value =
              "Congratulations! Your payment has been successfully processed. Thank you for choosing us.";
          ...
          paymentStatus.value = 'Failed';
          pDescription.value =
              "Oops! It seems like there was an issue processing your payment. Please check your payment details and try again.";
        } else {
          timeInSec.value = 10;
          isApi.value = false;
