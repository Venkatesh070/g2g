...
class OrderDetailsController extends GetxController {
  ...
  var isFromNotification = false.obs;
  ...
  void onInit() {
    ...
    // Check if coming from notification and store survey data
    isFromNotification.value = Get.arguments['fromNotification'] ?? false;
  ...
  Future<void> launchSupportPhone() async {
    ...
    final uri =
        phone.isEmpty ? Uri(scheme: 'tel') : Uri(scheme: 'tel', path: phone);
    if (await canLaunchUrl(uri)) {
      await launchUrl(uri);
    } else {
  ...
  funAddOrderRating(rating, comment) async {
    ...
    try {
      ...
      if (baseModel.success!) {
        ...
        // Check if survey data exists and navigate to survey
        if (surveyData != null || surveyId != null) {
          ...
          // If we have survey data, use it; otherwise fetch by surveyId
          if (surveyData != null) {
            surveyController.startSurveyFromRating(surveyData!, orderId);
          } else if (surveyId != null) {
            // Fetch survey data by ID
            final fetchedSurvey =
                await surveyController.fetchSurveyData(surveyId!);
            if (fetchedSurvey != null) {
              surveyController.startSurveyFromRating(fetchedSurvey, orderId);
            }
          }
        }
      } else {
...
// order_details_controller.dart
  void startPickupCountdown() {
    ...
    if (pickupDate == null || pickupTime == null || pickupEndTime == null)
      return;
    ...
    try {
      ...
      final pickupDateTime =
          CommonFunction.parsePickupDateTime(pickupDate, pickupTime);
      final pickupEndDateTime =
          CommonFunction.parsePickupDateTime(pickupDate, pickupEndTime);
      ...
      if (now.isBefore(pickupDateTime)) {
        ...
        pickupRemainingSeconds.value =
            pickupTotalSeconds.value.clamp(0, pickupTotalSeconds.value);
      ...
      pickupTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
        if (pickupRemainingSeconds.value > 0) {
          ...
          if (now.isAfter(pickupDateTime) && now.isBefore(pickupEndDateTime)) {
            isCountingToStart.value = false;
            pickupTotalSeconds.value =
                pickupEndDateTime.difference(now).inSeconds;
            pickupRemainingSeconds.value =
                pickupTotalSeconds.value.clamp(0, pickupTotalSeconds.value);
          } else {
            timer.cancel();
  ...
  Future<bool> funCancelOrder() async {
    ...
    try {
      ...
      if (orderCancelModel.success == true) {
        ...
        errorScreen(
            error: orderCancelModel.message ?? 'Failed to cancel order');
        return false;
      }
    } on CustomHttpException catch (exception) {
      progressDialog.dismiss();
